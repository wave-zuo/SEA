...
def parse_args():
    ...
    parser.add_argument('--input', default='./data/karate/karate_edges20',
...
if __name__ == '__main__':
    ...
    allcorrs = []
    for i in range(1):
        Matrix, Adj, Node = dataset.Read_graph_us2('data/ucsocial/compress/ucsocial_allcompress_dis_deg_layer1'+str(i))
        # Matrix, Adj, Node = dataset.Read_graph_us2('data/ussocial/compress/ussocial_allcompress_dis_deg_layer'+str(i))
        # samdata = pd.read_csv('data/ussocial/compress/ussocial_allcompress_sam_dis_deg_layer'+str(i)+'.csv')
        samdata = pd.read_csv('data/ucsocial/compress/ucsocial_allcompress_sam_dis_deg_layer1'+str(i)+'.csv')
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            loss_L2 += L_2nd
            loss_reg += L_reg
            # loss_value.append(loss_sum)
            scheduler.step(epoch)
            # print("The lr for epoch %d is %f" %(epoch, scheduler.get_lr()[0]))
            print("loss for epoch %d is: loss_sum is %f, loss_L1 is %f, loss_L2 is %f, loss_reg is %f" %(epoch, loss_sum,loss_L1,loss_L2,loss_reg))
        ...
        samdata['weight'] = np.exp(-1/samdata['weight'])
    ...
    print(allresults)
    print(allcorrs)
    print('c mean=' + str(np.mean(allcorrs)) + ', std=' + str(np.std(allcorrs)))
